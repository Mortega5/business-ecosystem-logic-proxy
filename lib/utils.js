...
exports.logOutgoing = function(logger, level, req, message, body) {
    const line = body !== undefined ? `${message}\n${JSON.stringify(body, null, 2)}` : message;
    logger[level]('%s - %s', req.id, line);
};
...
exports.getAPIPort = function(api) {
    ...
    ports[config.endpoints.software.path] = config.endpoints.software.port;
...
exports.getAPIHost = function(api) {
    ...
    hosts[config.endpoints.software.path] = config.endpoints.software.host;
...
exports.getAPIPath = function(api) {
    ...
    paths[config.endpoints.software.path] = config.endpoints.software.apiPath;
...
exports.getAPIProtocol = function(api) {
    ...
    protocols[config.endpoints.software.path] = config.endpoints.software.appSsl;
...
/**
 * Parses the raw request body as JSON and stores the result in req.parsedBody.
 * @param {Object} req - Incoming HTTP request whose body will be parsed
