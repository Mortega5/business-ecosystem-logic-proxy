...
/**
 * Appends relatedParty.id and relatedParty.role as query params to the outgoing backend
 * request, overwriting any values the client may have supplied. Both `req.query` and
 * `req.apiUrl` are updated so the params reach the backend (the NGSI-LD `q` param is not
 * used here since it is not supported by the context broker in use).
 *
...
exports.appendPartyRoleQuery = function(req, partyId, roleName) {
    req.query['relatedParty.id'] = partyId;
    req.query['relatedParty.role'] = roleName;
